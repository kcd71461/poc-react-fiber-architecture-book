...
## 다음 장 예고: Reconciliation — Fiber의 비교 알고리즘

다음 장에서는 Render Phase 내부에서 일어나는 핵심 계산,
즉 이전 트리와 새 트리를 비교해 최소한의 변경을 찾아내는 Reconciliation 알고리즘을 다룹니다.

- 일반 트리 비교(O(n³))를 O(n)으로 줄이는 휴리스틱
- `key`가 리스트 재조정 성능에 미치는 결정적 영향
- Reconciliation 결과가 Commit Phase의 DOM 조작 비용에 미치는 영향
